...
class NERModel(LanguageModel):
	...
	def load_data(self, debug=False):
		"""加载词向量（word-vectors）和train/dev/test 数据."""
		# 加载词向量
		...
		# Load the dev set (dev是作为辅助性训练的，防止过拟合)
	...
	def add_embedding(self):
		...
		"""
		# embedding lookup ：就是根据train_inputs中的id，寻找embeddings中的对应元素。比如，train_inputs=[1,3,5]，
		# 则找出embeddings中下标为1,3,5的向量组成一个矩阵返回
		# 定义优先使用gpu0执行
		with tf.device('/cpu:0'):
			# "/gpu:0"：表示机器中第一个GPU卡,同理"/cpu:0"：表示机器中第一个CPU；
			# TensorFlow中所有操作都有CPU和GPU运算的实现，默认情况下GPU运算的优先级比CPU高。
			# 如果TensorFlow操作没有指定在哪个设备上进行运算，默认会优选采用GPU进行运算
			### YOUR CODE HERE
			...
			# 在 L 中直接把window大小的context的word vector搞定
			window = tf.reshape(
		...
		with tf.variable_scope('Layer1', initializer=xavier_weight_init()) as scope:
			...
			if self.config.l2:  # L2 regularization for W
				tf.add_to_collection('total_loss',
									 0.5 * self.config.l2 * tf.nn.l2_loss(W))  # 0.5 * self.config.l2 * tf.nn.l2_loss(W)
		# L2正则化和用dropout来减小过拟合的处理
	...
	def add_loss_op(self, y):
		...
		loss = tf.add_n(tf.get_collection(
			'total_loss'))  # Adds all input tensors element-wise. inputs: A list of Tensor with same shape and type
		### END YOUR CODE
	...
	def __init__(self, config):
		...
		self.correct_predictions = tf.reduce_sum(
			tf.cast(correct_prediction, 'int32'))  # 3，关键步骤：将correct转换type，然后求和计算多少多的
	...
	def run_epoch(self, session, input_data, input_labels,
		...
		for step, (x, y) in enumerate(
			...
			loss, total_correct, _ = session.run(  # 前面都是定义，这里才是执行
					...
					# 1.喂进去x y dropout，执行 loss，correct_predictions，train_op
...
def test_NER():
	...
	您可以使用此函数来测试命名实体的实现
	识别网络。 调试时，将Config对象中的max_epochs设置为1
	所以你可以快速迭代。
	"""
	config = Config()
	with tf.Graph().as_default():
		...
		with tf.Session() as session:
			best_val_loss = float('inf')  # 初始化一个最大的float值，inf代表正无穷
			best_val_epoch = 0
			...
			for epoch in xrange(config.max_epochs):
				...
				train_loss, train_acc = model.run_epoch(session, model.X_train,
														model.y_train)
				# 1.把 train 数据放进迭代里跑，得到 损失loss 和 准确率accuracy
				val_loss, predictions = model.predict(session, model.X_dev,
													  model.y_dev)
				# 2.用这个model去预测 dev 数据，得到loss 和 prediction
				print ('训练 loss: {}'.format(train_loss))
				print ('训练 acc: {}'.format(train_acc))
				print ('验证 loss: {}'.format(val_loss))
				...
				# 把最小的 loss 对应的 weights 保存起来
				if epoch - best_val_epoch > config.early_stopping:
					break
				###
				confusion = calculate_confusion(config, predictions,
												model.y_dev)
				# 3.把 dev 的lable数据放进去，计算预测prediction的confusion
